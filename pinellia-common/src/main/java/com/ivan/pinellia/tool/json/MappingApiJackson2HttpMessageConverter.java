package com.ivan.pinellia.tool.json;
...
public class MappingApiJackson2HttpMessageConverter extends AbstractReadWriteJackson2HttpMessageConverter {
	...
	private static ObjectMapper initWriteObjectMapper(ObjectMapper readObjectMapper) {
		...
		// null 处理
		writeObjectMapper.setSerializerFactory(writeObjectMapper.getSerializerFactory().withSerializerModifier(new PinelliaBeanSerializerModifier()));
		writeObjectMapper.getSerializerProvider().setNullValueSerializer(PinelliaBeanSerializerModifier.NullJsonSerializers.STRING_JSON_SERIALIZER);
